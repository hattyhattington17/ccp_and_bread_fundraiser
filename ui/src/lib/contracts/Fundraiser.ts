...
import { FungibleToken, VKeyMerkleMap, SideloadedProof } from 'fts-scaffolded-xt';
...
const EMPTY_INDEXED_TREE4_ROOT =
	Field(848604956632493824118771612864662079593461935463909306433364671356729156850n);
...
export class Fundraiser extends SmartContract {
	...
	async donate(
		...
		this.network.timestamp
			.getAndRequireEquals()
			.assertLessThan(this.deadline.getAndRequireEquals(), 'deadline has passed');
	...
	async refundDonation(state: MerkleMap) {
		...
		this.network.timestamp
			.getAndRequireEquals()
			.assertGreaterThan(this.deadline.getAndRequireEquals(), 'deadline not reached');
